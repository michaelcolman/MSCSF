...
int main(int argc, char *argv[])
{
    ...
    while (counter < argc)
    {
        ...
        else if (strcmp(argv[counter], "Write_regions") == 0)
        {
            if (strcmp(argv[counter+1], "On") == 0) write_regions = true;
            else if (strcmp(argv[counter+1], "Off") == 0) write_regions = false;
            else
            {
                printf("ERROR: Write_regions can only be On or Off\n");
            }
            counter++;
        }
    ...
    else if (strcmp(model_type, "native_network") == 0) 
    {
    ...
    else if (strcmp(model_type, "integrated_network") == 0)
    {
    ...
    {
        printf("ERROR: model_type \"%s\" is not valid: chose only \"native\" or \"integated\" or \"native_network\" or \"integated_network\"\n");
        exit(1);
    }
    ...
    for (iteration = start_time; iteration <= end_time; iteration += interval)
    {
        ...
        if (write_vtk == true) 
        {
            ...
            if (write_regions == true)
            {
                for (int i = 1; i < Tissue.Ncelltypes+1; i++)
                {
                    vtk_3D_output_region_specific(variable, directory, sr_dir, V, SC, iteration, i);
                }
            }
        }
