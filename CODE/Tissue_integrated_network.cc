...
int main(int argc, char *argv[])
{
	...
	call_argument_functions(argc, argv, &Argin, "Tissue_integrated_network"); // Reads settings file and calls set args || lib/Arguments.c
	...
	if (Argin.reference_arg == true) sprintf(directory, "Outputs_0Dtissue_network_%s", Sim.reference);
	else sprintf(directory, "Outputs_0Dtissue_network");
    ...
    if (Sim.Mac == true || Sim.Linux == true)   sprintf(mkfile, "%s/%s/CRU_cell1.txt", directory, results_dir);
    else if (Sim.Windows == true)               sprintf(mkfile, "%s\\%s\\CRU_cell1.dat", directory, results_dir);
    ofstream out_cru1(mkfile);    // Contains all CRU related outputs
    printf("\t %s\n", mkfile);
    ...
    if (Sim.Mac == true || Sim.Linux == true)   sprintf(mkfile, "%s/%s/CRU_cell2.txt", directory, results_dir);
    else if (Sim.Windows == true)               sprintf(mkfile, "%s\\%s\\CRU_cell2.dat", directory, results_dir);
    ofstream out_cru2(mkfile);    // Contains all CRU related outputs
    printf("\t %s\n", mkfile);
    ...
    if (Sim.Mac == true || Sim.Linux == true)   sprintf(mkfile, "%s/%s/CRU_cell3.txt", directory, results_dir);
    else if (Sim.Windows == true)               sprintf(mkfile, "%s\\%s\\CRU_cell3.dat", directory, results_dir);
    ofstream out_cru3(mkfile);    // Contains all CRU related outputs
    printf("\t %s\n", mkfile);
	...
	else if (strcmp(Params_global.Model, "mCRN") == 0);	    // dog atrial cell; Varela et al 2016 PLOS Computational Biology 12(12): e1005245
	else
	{
	...
	cell3ref = SC.N - 5;
     ...
     // setup diffusion coefficient arrays NETWORK
    if (strcmp(Tissue.apply_symmetry_factor, "On") == 0)
    set_G_dx_global(&SC, Tissue.dx, Tissue.dy, Tissue.dz, Tissue.Gl, Tissue.Gt, Tissue.Gt2, Tissue.symm_fac_diag_long, Tissue.symm_fac_diag_trans, Tissue.symm_fac_corner);   // sets gt and gl from tissue model || lib/Spatial_coupling.cpp
    else set_G_dx_global(&SC, Tissue.dx, Tissue.dy, Tissue.dz, Tissue.Gl, Tissue.Gt, Tissue.Gt2, 1.0, 1.0, 1.0);
	...
	// Baseline, non-uniform Dscale (celltype or map; map for geo only) NETWORK
    // This is for regional or continuous/complex gradient in D1 and/or DAR (inherehent to tissue model)
    if (strcmp(Tissue.D_uniformity, "uniform") != 0) update_G_arrays_Dscale_baseline(&SC, &Tissue,PATH, directory); // lib/Tissue.cpp

    // Modification Dscale (homogeneous or map; ideal or geo) NETWORK
    // This is for scaling D1 and/or DAR locally associated with modulation (e.g. remodelling)
    update_G_arrays_Dscale_mod(&SC, &Tissue, PATH, directory); // lib/Tissue.cpp

    // NETWORK set the weights
    set_gGgap_array(&SC, Tissue.Orientation_type);  // lib/Spatial_coupling.cpp

    // NETWORK maps
    set_gjunc_and_junc_maps(&SC, directory, &Tissue);    // lib/Spatial_coupling.cpp
    for (int n = 0; n < SC.N; n++) SC.diff[n] = 0;              // zero diff initially (important as zeroed after calculated in time loops)
    printf("\tJunction maps made\n");

    // Now apply any modifications from a baseline or mod map that applies directly to the network connections (junctions not cells)
    default_junction_maps(&SC); // lib/Spatial_coupling.cpp -> sets Junction mod map to 1 as a default
    printf("\tJunction modulation maps defaulted\n");

    // Modify junctions based on baeline (het) and modulation map (e.g. for fibrosis)
    if (strcmp(Tissue.junction_base_map_on, "On") == 0)
    {
        read_map_file_Njunc(&SC, Tissue.junction_base_map_file, "Tissue_geometries", PATH, directory, SC.gGgap_base_map);
    }
    if (strcmp(Tissue.junction_mod_map_on, "On") == 0)
    {
        read_map_file_Njunc(&SC, Tissue.junction_mod_map_file, "Tissue_geometries", PATH, directory, SC.gGgap_mod_map);
    }

    update_junctions(&SC);                                      // modifies junctions based on map (should just multiply by 1 if no maps used)
    output_junc_maps(&SC, directory, &Tissue);                  // Spatial coupling - writes connection map after junctions have been removed
    printf("\tJunction modulation maps applied and output\n");
	...
	for (int n = 0; n < SC.N; n++)
	{
		...
		if (strcmp(Params_global.Ca_cellular_het, "On") == 0) update_heterogeneity_and_modulation_integrated(&Params[n]); // lib/Model.c
    ...
    if (strcmp(Argin.SRF_mode, "Read") == 0)
    {
        if (Argin.SRF_Read_filename_arg == false)
        {
            cout << "ERROR!: SRF is set to read, but no filename to read from has been set" << endl;
            exit(1);
        }
        for (int n = 0; n < SC.N; n++) SRF[n].Mode = "Off"; // default all to Off, so that if no SRF for node n in file, that node won't do anything
        read_SRF_settings_from_file(SRF, Argin.SRF_Read_filename); // lib/Spontaneous_release_functions.cpp -> reads from file into waveform settings directly (Mode set to read in this function)
        printf("Spontaneous release function parameters read.\n");
    }
    else
    {
        for (int n = 0; n < SC.N; n++)
    ...
    if (strcmp(Sim.Read_state, "On") == 0)        // Reads whole tissue
    {
        ...
        Read_state_tissue_integrated_net_whole_tissue(State, Params, Sim.BCL, PATH, Params_global.Model, SC.N, Tissue.Tissue_order, Tissue.Tissue_model, Tissue.Tissue_type, Tissue.Orientation_type, Sim.state_reference_read); //lib/Read_write_state.c
        for (int n = 0; n < SC.N; n++)
        {
    ...
    else if (strcmp(Sim.Read_state, "Off") != 0) // other versions of read state
    {
        for (int n = 0; n < SC.N; n++)
        {
            ...
            else if (strcmp(Sim.Read_state, "ave") == 0)		Read_state_tissue_integrated_net_ave_tissue(&State[n], Params[n], Sim.BCL, PATH, Params[n].Model, Sim.state_reference_read); 	//lib/Read_write_state.c 
	...
	for (sim_time = 0.0; sim_time <= (float)Sim.Total_time; sim_time += Sim.dt)
	{
        ...
        // Loop over all tissue and set SC.diff to 0, so can be added to below (NETWORK)
#pragma omp parallel for default(none) shared(SC, Vm, State)
        ...
        // Loop over all gap junctions (NETWORK) -> sets SC.diff
#pragma omp parallel for default(none) shared(SC, Vm, State)
        for (int m = 0; m < SC.Njunc; m++)
        {
            calc_IGap(&SC, Vm, m);     // lib/Spatial_coupling.cpp
        }
...
#pragma omp parallel for default(none) shared(SC, Vm, State, Cai, CaSR)
        ...
        if (iteration_counter%(50*(Variables[0].dtinv)) == 0) // as 50 is less than time between successive SRF, we only need to sample at 50 ms intervals
    ...
    if (strcmp(Sim.Write_state, "On") == 0) // whole tissue dump
    {
        ...
        Write_state_tissue_integrated_net_whole_tissue(State, Params, Sim.BCL, PATH, Params_global.Model, SC.N, Tissue.Tissue_order, Tissue.Tissue_model, Tissue.Tissue_type, Tissue.Orientation_type, Sim.state_reference_write); //lib/Read_write_state.c
    ...
    else if (strcmp(Sim.Write_state, "ave") == 0) // writes state for just one cell in the tissue (for region x)
    {
        ...
        else 
        {
            ...
            Write_state_tissue_integrated_net_ave_tissue(State[10], Params[10], Sim.BCL, PATH, Params[10].Model, Sim.state_reference_write); //lib/Read_write_state.c
        }
	...
	SC_array_deallocation_Njunc(&SC);
