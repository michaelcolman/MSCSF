...
void set_argument_defaults(Argument_parameters *A)
{
    ...
    //NETWORK
    A->junction_mod_map_on_arg          = false;
    A->junction_mod_map_file_arg        = false;
    A->junction_base_map_on_arg         = false;
    A->junction_base_map_file_arg       = false;
    A->apply_symmetry_factor_arg        = false;
	// End Tissue settings ==========//|
	...
	A->SRF_het_arg			    = false;
    A->write_SRF_arg            = false;
	A->SRF_N_DC_set		    	= 0;
	A->SRF_N_Dyn_set		    = 0;
    A->SRF_Read_filename_arg    = false;

    A->SRF_mode                 = "Off";

    A->t_to_peak_dist_scale         = 1;
    A->t_to_peak_dist_scale_arg     = false;
    A->NRyRo_peak_dist_scale        = 1;
    A->NRyRo_peak_dist_scale_arg    = false;
    A->NRyRo_plateau_dist_scale     = 1;
    A->NRyRo_plateau_dist_scale_arg = false;
	// Spontaneous release functions //|
...
void set_arguments(int Narg, char *argin[], Argument_parameters *A, char const *Version, FILE *out)
{
	...
	while (counter < Narg) // loop until Number of arguments has been reached
	{
		...
		// Model and cell conditions ========================================\\|
		if (strcmp(argin[counter], "Model") == 0 || strcmp(argin[counter], "model") == 0)
		{
        ...
        if (strcmp(argin[counter], "Ca_model_cellular_het") == 0)
        {
            A->Ca_cellular_het              = argin[counter+1];
            A->Ca_cellular_het_arg          = true;
            fprintf(out, "Ca_model_cellular_het %s ", argin[counter+1]);
            if (strcmp(A->Ca_cellular_het, "On") != 0 && strcmp(A->Ca_cellular_het, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid Ca_cellular_het argument. Please pass only \"Off\" or \"On\"\n\n", A->Ca_cellular_het);
                exit(1);
            }
            counter++; isFound = true;
        }
		...
		if (strcmp(argin[counter], "remod_map_file") == 0 || strcmp(argin[counter], "remodelling_map_file") == 0)
        {
        ...
        // NETWORK
        if (strcmp(argin[counter], "Junction_modulation_map") == 0)
        {
            A->junction_mod_map_on          = argin[counter+1];
            A->junction_mod_map_on_arg      = true;
            fprintf(out, "Junction_modulation_map   %s ", argin[counter+1]);
            if (strcmp(A->junction_mod_map_on, "On") != 0 && strcmp(A->junction_mod_map_on, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid Junction_modulation_map argument. Please pass only \"Off\" or \"On\"\n\n", A->junction_mod_map_on);
                exit(1);
            }
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "Junction_modulation_map_file") == 0)
        {
            A->junction_mod_map_file      = argin[counter+1];
            A->junction_mod_map_file_arg  = true;
            fprintf(out, "Junction_modulation_map_file %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "Junction_baseline_map") == 0)
        {
            A->junction_base_map_on          = argin[counter+1];
            A->junction_base_map_on_arg      = true;
            fprintf(out, "Junction_baseline_map   %s ", argin[counter+1]);
            if (strcmp(A->junction_base_map_on, "On") != 0 && strcmp(A->junction_base_map_on, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid Junction_baseline_map argument. Please pass only \"Off\" or \"On\"\n\n", A->junction_mod_map_on);
                exit(1);
            }
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "Junction_baseline_map_file") == 0)
        {
            A->junction_base_map_file      = argin[counter+1];
            A->junction_base_map_file_arg  = true;
            fprintf(out, "Junction_modulation_map_file %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "Apply_symmetry_factors") == 0)
        {
            A->apply_symmetry_factor          = argin[counter+1];
            A->apply_symmetry_factor_arg      = true;
            fprintf(out, "Apply_symmetry_factors   %s ", argin[counter+1]);
            if (strcmp(A->apply_symmetry_factor, "On") != 0 && strcmp(A->apply_symmetry_factor, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid Apply_symmetry_factors argument. Please pass only \"Off\" or \"On\"\n\n", A->apply_symmetry_factor);
                exit(1);
            }
            counter++; isFound = true;
        }
        ...
        if (strcmp(argin[counter], "LTCC_het_map_file") == 0)
        {
        ...
        if (strcmp(argin[counter], "Write_SRF_settings") == 0)
        {
            A->write_SRF_settings   = argin[counter+1];
            A->write_SRF_arg       = true;
            fprintf(out, "Write_SRF_settings %s ", argin[counter+1]);
            if (strcmp(A->write_SRF_settings, "On") != 0 && strcmp(A->write_SRF_settings, "Off") != 0)
            {
                printf("ERROR: \"%s\" is not a valid Write_SRF_settings argument. Please pass only \"Off\" or \"On\"\n\n", A->write_SRF_settings);
                exit(1);
            }
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "SRF_read_filename") == 0 || strcmp(argin[counter], "SRF_Read_filename") == 0)
        {
            A->SRF_Read_filename        = argin[counter+1];
            A->SRF_Read_filename_arg    = true;
            fprintf(out, "SRF_read_filename %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        ...
        // t to peak and peak RyR random variation amount
        if (strcmp(argin[counter], "SRF_t_to_peak_variation") == 0)
        {
            A->t_to_peak_dist_scale     = atof(argin[counter+1]);
            A->t_to_peak_dist_scale_arg = true;
            fprintf(out, "t_to_peak_variation %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "SRF_NRyRo_variation") == 0)
        {
            A->NRyRo_peak_dist_scale     = atof(argin[counter+1]);
            A->NRyRo_peak_dist_scale_arg = true;
            fprintf(out, "SRF_NRyRo_variation %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        if (strcmp(argin[counter], "SRF_NRyRo_plateau_variation") == 0)
        {
            A->NRyRo_plateau_dist_scale     = atof(argin[counter+1]);
            A->NRyRo_plateau_dist_scale_arg = true;
            fprintf(out, "SRF_NRyRo_plateau_variation %s ", argin[counter+1]);
            counter++; isFound = true;
        }
        ...
        if (!isFound)
		{
			...
			if (strcmp(Version, "Tissue_native") == 0 || strcmp(Version, "Tissue_integrated") == 0)
			{
                ...
                printf("\tFull list of tissue models available in this implementation\n");
                printf("\t\t1D, 2D and 3D idealised (Tissue_order 1D/2D/3D):\n");
                printf("\t\t\tfunctional_model_test\n");
                printf("\t\t\tbasic\n");
                printf("\t\t\tconduction_velocity\n");
                printf("\t\t\tfibre_orientation\n");
                printf("\t\t\tre-entry\n");
                printf("\t\t\tre-entry_large\n");
                printf("\t\t\tVent_transmural\n");
                printf("\t\t\tHeterogeneous_human_atria\n");
                printf("\t\tgeo:\n");
                printf("\t\t\tfunctional_model_test\n");
                printf("\t\t\tHuman_vent_wedge\n");
                printf("\t\t\tCanine_vent\n");
                printf("\t\t\tVector_field_and_fibrosis_2D_vis\n");
                printf("\t\t\tVector_field_and_fibrosis_2D_sims\n");
                printf("\t\t\tVector_field_and_fibrosis_3D\n");
                printf("\t\t\tRat_vent_three_eigenvectors_control_base\n");
                printf("\n");
			}
			...
			if (strcmp(Version, "Single_cell_3D") == 0)
            {
                ...
                printf("\tSERCA_map_file [string]\tNCX_map_file [string]\tRyR_het_map_file [string]\tLTCC_het_map_file [string]\n");
