...
int main(int argc, char *argv[])
{
    ...
    for (iteration = start_time; iteration <= end_time; iteration += interval)
    {
        ...
        if (write_slices== true)
        {
            sprintf(filename_out, "%s/%s/%s_output_2D_XYslice_z_%d_time_%04d.dat", directory, sr_dir, variable, z, iteration);
            printf("Creating visualisation file %s\n", filename_out);
            data_2D_XYslice_output(variable, directory, sr_dir, V, SC, iteration, z);

            sprintf(filename_out, "%s/%s/%s_output_2D_XZslice_y_%d_time_%04d.dat", directory, sr_dir, variable, y, iteration);
            printf("Creating visualisation file %s\n", filename_out);
            data_2D_XZslice_output(variable, directory, sr_dir, V, SC, iteration, y);

            sprintf(filename_out, "%s/%s/%s_output_2D_YZslice_x_%d_time_%04d.dat", directory, sr_dir, variable, x, iteration);
            printf("Creating visualisation file %s\n", filename_out);
